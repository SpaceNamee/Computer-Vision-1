...
parser.add_argument("train_pct", help="Ratio of images to go to train folder; the rest go to validation folder.", default=.8)
...
if train_percent < .01 or train_percent > 0.99:
    print("Invalid entry for train_pct. Please enter a number between")
    sys.exit(0)
...
# Define path to image and annotation folders
cwd = os.getcwd() # return the Current Working Directory (CWD) of the process (Absolute Path)
...
img_file_list = [path for path in Path(input_image_path).rglob("*")] # Return Type: It returns a generator, which is memory-efficient because it yields paths one by one rather than loading everything into a list at once
...
# Select files randomly and copy them train or val folder
for i, set_num in enumerate([train_num, val_num]):
    for ii in range(set_num):
        img_path = random.choice(img_file_list)
        img_fn = img_path.name # full name
        base_fn = img_path.stem # only name without for example .txt
        txt_fn = base_fn + ".txt"
